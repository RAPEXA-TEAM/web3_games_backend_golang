...
func main() {
	//centrifugo.StartCentrifugo()
	go func() {
		log.Println(http.ListenAndServe(":6060", nil))
	}()

	client := centrifuge.NewJsonClient(
		"ws://localhost:"+centrifugo.CENTRIFUGO_PORT+"/connection/websocket",
		centrifuge.Config{
	...
	go func(sub *centrifuge.Subscription) {
		...
		for {
			...
			case "#history":
				result, err := sub.History(context.Background(), centrifuge.WithHistoryLimit(5))
				for i := range result.Publications {
					println(string(result.Publications[i].Data[:]))
				}
				if err != nil {
					log.Println(err)
				}
